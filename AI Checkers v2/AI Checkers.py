    ...
    def make_move(self):
        ...
        if len(can_eat) > 0:
            ...
            if self.adaptive_brain is not None:
                inputs = self.board.get_neural_input()
...
def integer_to_real_mapper(integers):
    ...
    # in the range of 0 - 8191 (1/8191 * 8191 = 1) AND (1/8191 * 0 = 0)
...
def eval_genome_inefficient(genomes, config):
    ...
    really_inefficient_tournament(stripped_genomes, config, current_top_brain, c_board, generation_number, 20)
...
def train_for_set_number_generations(print_each_generation, save_to_file):
    ...
    city.print_population_real()
    if save_to_file:
        top_mind = Mind(checkerboard, 1, city.population[0].get_real_numbers())
        top_mind.save_base_brain()
...
def train_adaptive_brain(config, save_to_file):
    ...
    winning_genome = new_city.run(eval_genome_inefficient)
...
# train_until_set_fitness(True, True)
